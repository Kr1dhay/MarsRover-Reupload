...
String rockDetected(){
  ...
  else if(rfSensor.isAdamantine() && readMag()){
    return "Adamantine";
  }
  else if(rfSensor.isXirang() && !readMag()){
    return "Xirang";
  }
...
void loop() 
{
  ...
  if (packetSize)
  {
    ...
    if (packetBuffer[packetSize - 1] == 'w'){ // forward  
      digitalWrite(0,1); 
    }
    else if(packetBuffer[packetSize - 1] == 's'){ // backward
      analogWrite(LPWM,255);
      analogWrite(RPWM,255);
      digitalWrite(LDIR,1);
      digitalWrite(RDIR,1);
    }
    else if(packetBuffer[packetSize - 1] == 'a'){ // left
      analogWrite(LPWM,255);
      analogWrite(RPWM,255);
      digitalWrite(LDIR,0);
      digitalWrite(RDIR,1);
    }
    else if(packetBuffer[packetSize - 1] == 'd'){ // right
      analogWrite(LPWM,255);
      analogWrite(RPWM,255);
      digitalWrite(LDIR,1);
      digitalWrite(RDIR,0);
    }
    else if(packetBuffer[packetSize - 1] == '1'){ // stop (executed upon key release)
      digitalWrite(0,1);
    }
    else if(packetBuffer[packetSize - 1] == 'f'){ // detect
      Udp.beginPacket(Udp.remoteIP(), 55125);
      
      // Udp.write(rockDetected());
